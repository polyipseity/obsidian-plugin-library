...
- Reveal machinery: the type-level semantics of `RevealPrivate`, the `RevealWhitelist` whitelist (private types `$X` to reveal) and the `RecursionBlacklist` blacklist (types to stop recursing), plus the configurable `Depth` number generic (recursion bound, orthogonal to both), the exempt marker policy, and the empty `$BakedHotkey` brand policy are specified in `.agents/instructions/reveal-private.instructions.md` and the docstrings of `src/private.ts`. Type tests live in the "Type system" describe block of `tests/src/private.spec.ts`.
