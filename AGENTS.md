...
- Reveal machinery: the type-level semantics of `RevealPrivate`, the exact-whitelist `Filter`, the exempt marker policy, and the `BakedHotkey` workaround ban are specified in `docs/reveal-private.md`. Type tests live in the "Type system" describe block of `tests/src/private.spec.ts`.
