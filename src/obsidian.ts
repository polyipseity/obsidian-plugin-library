...
export class LambdaComponent extends Component {
  public constructor(
    protected readonly onLoad = function fn(this: LambdaComponent): void {
      // Noop
    },
    protected readonly onUnload = function fn(this: LambdaComponent): void {
      // Noop
    },
  ) {
    super();
...
export abstract class ResourceComponent<T> extends Component {
  ...
  public get onLoaded(): Promise<T> {
    return (this.#loader as PromisePromise<T>).then(
      async ({ promise }) => promise,
    );
  }

  public get value(): T {
    if (this.#value === ResourceComponent.sentinel) {
      throw new Error();
    }
    return this.#value;
  }
...
export class UpdatableUI {
  ...
  public newSetting(
    ...
    return this.new(
      ...
      (setting) => {
        configure(
          setting.setDesc("").setDisabled(false).setName("").setTooltip(""),
        );
        recording = false;
      },
      (setting) => {
...
export class UnnamespacedID<V extends string> {
  ...
  public namespaced(context: Plugin | PluginManifest): string {
    return `${
      (context instanceof Plugin ? context.manifest : context).id
    }:${this.id}`;
  }
}
...
export async function awaitCSS(element: HTMLElement): Promise<void> {
  const {
    classList,
    style,
    style: { display },
...
export function notice(
  ...
  const unreg = context.language.onChangeLanguage.listen(() =>
