...
> extends ResourceComponent<DeepReadonly<T>> {
  ...
  public constructor(protected readonly fixer: Fixer<T>) {
    super();
  }

  public async mutate(
    mutator: (settings: DeepWritable<T>) => unknown,
  ): Promise<void> {
  ...
  public async read(
    reader: () => unknown = (): ReturnType<typeof this.read0> => this.read0(),
  ): Promise<void> {
  ...
  public onMutate<V>(
    ...
    callback: (cur: V, prev: V, settings: DeepReadonly<T>) => unknown,
  ): () => void {
    let prev = accessor(this.value);
    return this.#onMutate.listen(async (): Promise<void> => {
  ...
  async #read(
    reader: () => unknown = (): ReturnType<typeof this.read0> => this.read0(),
  ): Promise<DeepWritable<T>> {
    const { fixer } = this,
      read = await reader(),
      { value, valid } = fixer(read);
    if (!isNil(read) && !valid) {
      await this.onInvalidData(read, value);
    }
    return value;
  }
...
export class StorageSettingsManager<
  T extends StorageSettingsManager.Type,
> extends AbstractSettingsManager<T> {
  ...
  protected override async onInvalidData(
    ...
    fixed.recovery[
      `${StorageSettingsManager.RECOVERY_PREFIX}${new Date().toISOString()}`
...
export namespace StorageSettingsManager {
  ...
  export function fix(self0: unknown): Fixed<Type> {
    ...
    return markFixed(self0, {
      ...
      recovery: Object.fromEntries(
        Object.entries(launderUnchecked(unc.recovery)).map(([key, value]) => [
          key,
          String(value),
        ]),
      ),
  ...
  export function setRecovery(
    ...
    for (const key of Object.keys(recovery)) {
      ...
      // eslint-disable-next-line @typescript-eslint/no-dynamic-delete -- clean up recovery entries
      delete recovery[key];
    }
    Object.assign(recovery, Object.fromEntries(map));
  }
  export function hasFailed(value: Type): boolean {
    return FAILED in value;
  }
}

export class SettingsManager<
  T extends SettingsManager.Type,
> extends AbstractSettingsManager<T> {
  readonly #write = asyncDebounce(
    throttle((resolve: (value: AsyncOrSync<void>) => void) => {
  ...
  protected override async onInvalidData(
    ...
    const {
      context,
      context: { language, localSettings },
    } = this;
    await Promise.all([
      (async (): Promise<void> => {
        await language.onLoaded;
      ...
      (async (): Promise<void> => {
        try {
          ...
          await localSettings.mutate((lsm) => {
            lsm.recovery[
              `${SettingsManager.RECOVERY_PREFIX}${new Date().toISOString()}`
            ] = JSON.stringify(actual, null, JSON_STRINGIFY_SPACE);
          });
...
export function registerSettingsCommands(context: PluginContext): void {
  const {
    app,
    app: { fileManager, metadataCache, workspace },
    language: { value: i18n },
    settings,
  } = context;
  ...
  addCommand(context, () => i18n.t("commands.export-settings-current-file"), {
    checkCallback(checking) {
      ...
      if (file?.extension !== FileExtensions.MARKDOWN) {
        return false;
      }
      if (!checking) {
        const cachedFm = cleanFrontmatterCache(
            ...
            fileManager
              ...
              .catch((error: unknown) => {
                printError(
                  anyToError(error),
                  () =>
                    i18n.t("errors.error-processing-frontmatter", {
                      file,
  ...
  addCommand(context, () => i18n.t("commands.import-settings-current-file"), {
    checkCallback(checking) {
      const file = workspace.getActiveFile();
      if (file?.extension !== FileExtensions.MARKDOWN) {
        return false;
