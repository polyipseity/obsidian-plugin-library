...
>(
  ...
  return (component: C): void => {
    ...
    if (post) {
      post(component, activate);
    }
  };
}

export function composeSetters<V, C extends ValueComponent<V>>(
  ...setters: readonly ((value: V, component: C, getter: () => V) => unknown)[]
) {
  return async (value: V, component: C, getter: () => V): Promise<boolean> => {
...
export function resetButton<C extends ButtonComponent | ExtraButtonComponent>(
  ...
  return (component: C): void => {
    ...
    component.setIcon(icon).setTooltip(tooltip).onClick(activate);
    if (post) {
      post(component, activate);
    }
  };
    ...
    unexpected,
    {
      ...
      pre(component) {
        component
          ...
          .addOptions(
            Object.fromEntries(
              selections.map((selection, index) => [index, selection.name]),
            ),
          );
        const { pre } = action;
        if (pre) {
