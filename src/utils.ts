...
export class EventEmitterLite<A extends readonly unknown[]> {
  ...
  public async emit(...args: A): Promise<void> {
    return new Promise((resolve, reject: (reason?: unknown) => void) => {
      this.lock
        .acquire(EventEmitterLite.emitLock, async () => {
          // Copy to prevent concurrent modification
          const emitted = [...this.#listeners].map(async (list) => {
            await list(...args);
          });
...
> {
  ...
  public transform(
    func: (self0: this[number][]) => readonly this[number][],
  ...
  public call(
    ...args: Args
    // eslint-disable-next-line @typescript-eslint/no-invalid-void-type -- conditional return type for void
  ...
  public call0(
    thisArg: unknown,
    ...args: Args // eslint-disable-next-line @typescript-eslint/no-invalid-void-type -- conditional return type for void
  ): Async extends true ? Promise<void> : Async extends false ? void : never;
...
>(self0: T, key: K & keyof T, value?: T[K]): typeof value {
  if (value === void 0) {
    // eslint-disable-next-line @typescript-eslint/no-dynamic-delete -- clean up unused key in target object
    delete self0[key];
  } else {
    self0[key] = value;
  }
...
export function asyncDebounce<A extends readonly unknown[], R>(
  func: (
...
export function bracket<T extends object, K extends string | number | symbol>(
  self0: T,
  key: K,
):
  ...
  return Object.freeze(
    proof ? { valid: true, value: proof() } : { valid: false },
  );
}
...
export function capitalize(str: string, locales?: string[] | string): string {
...
export function clearProperties(self0: object): void {
  for (const key of typedOwnKeys(self0)) {
    // eslint-disable-next-line @typescript-eslint/no-dynamic-delete -- clean up object properties
    delete self0[key];
  }
}
...
export function escapeJavaScriptString(value: string): string {
  return `\`${value.replace(/(?<char>`|\\|\$)/gu, "\\$<char>")}\``;
}
...
export function typedIn<T extends object, K extends string | number | symbol>(
  self0: T,
  key: K,
): (() => T[K & keyof T]) | null {
  if (key in self0) {
...
export function typedOwnKeys<T extends object>(
  ...
  return Reflect.ownKeys(self0) as (keyof T & (string | number | symbol))[];
}
...
export async function promisePromise<T = void>(): PromisePromise<T> {
  return new Promise((resolve0) => {
    const promise = new Promise<T>((resolve, reject) => {
      resolve0(Promise.resolve().then(() => ({ promise, reject, resolve })));
    });
  });
}
...
export function uncapitalize(str: string, locales?: string[] | string): string {
