...
export class ListModal<T> extends Modal {
  ...
  public constructor(
    ...
    this.#editables = deepFreeze([
      ...(options?.editables ?? ListModal.EDITABLES),
    ]);
...
export class EditDataModal<T extends object> extends Modal {
  ...
  public constructor(
    ...
    this.#callback = options?.callback ?? ((): void => {});
    this.#elements = deepFreeze([
      ...(options?.elements ?? EditDataModal.ELEMENTS),
    ]);
    this.#title = options?.title;
    this.#description = options?.description;
  }
      ...
      ui.newSetting(element, (setting) => {
        setting
          ...
          .addButton((button) => {
            ...
            button
              .setIcon(
                i18n.t("asset:components.edit-data.export-to-clipboard-icon"),
              )
              .setTooltip(i18n.t("components.edit-data.export-to-clipboard"))
              .onClick(async () => {
                try {
                  await activeSelf(buttonEl).navigator.clipboard.writeText(
                    this.#dataText,
                  );
                } catch (error) {
      ...
      ui.newSetting(element, (setting) => {
        setting
          ...
          .addButton((button) => {
            ...
            button
              .setIcon(
                i18n.t("asset:components.edit-data.import-from-clipboard-icon"),
              )
              .setTooltip(i18n.t("components.edit-data.import-from-clipboard"))
              .onClick(async () => {
                try {
                  const { value: parsed, valid } = fixer(
                    JSON.parse(
...
export namespace EditDataModal {
  export const ELEMENTS = deepFreeze(["export", "import", "data"]);
  export interface Options<T> {
        ...
        () =>
          scope.register([], "enter", async (event) => {
            if (preconfirmed) {
              ...
              confirmButton
                ?.setCta()
                .buttonEl.classList.remove(DOMClasses.MOD_WARNING);
            }
            consumeEvent(event);
          }),
        null,
